...
def generate_csv_from_dyna():
    ...
    df["ts"] = dyna_df["time"]
    df["label"] = dyna_df["volume"]
    df["f0"] = dyna_df["volume"]
...
def edge_feat():
    ...
    edge_df.to_csv(
        f"raw_data/{dataset_name}/ml_{dataset_name}_edge_feats.csv", index=False
    )
...
def read_edge_feats():
    edge_feats = np.load(
        f"raw_data/{dataset_name}/ml_{dataset_name}_edge_feats.npy", allow_pickle=True
    ).item()
    print(edge_feats)
