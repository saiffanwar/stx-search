...
class BaseExplainerTG(object):
    def __init__(self, model_name: str, explainer_name: str, dataset_name: str, all_events: str, explanation_level: str, device,
                verbose: bool = True, results_dir: Optional[str] = None, debug_mode: bool=True,
                 edge_feat = None, input_window = None
                 ) -> None:
        """
...
#        self.model.to(self.device)
        ...
        self.tgnn_reward_wraper = TGNNRewardWraper(self.model_name, self.all_events, self.explanation_level)
...
#                num_neighbors = self.model.num_neighbors # NOTE: important
            ...
            for i in range(num_layers):
                ...
                out_ngh_node_batch, out_ngh_eidx_batch, out_ngh_t_batch = self.ngh_finder.get_temporal_neighbor(
                                                                                    last_nodes,
                                                                                    last_ts,
...
#        self.candidate_events, unique_e_idx = self.find_candidates(event_idx, threshold_num=threshold_num, num_neighbors=200)
        self.candidate_events, unique_e_idx = self.libcity_base_explainer.set_computation_graph(self.all_events, self.edge_feat, event_idx)
...
#        assert hasattr(self, 'ori_subgraph_df')
        self.base_events = self.candidate_events
...
#        self.tgnn_reward_wraper.compute_original_score(self.computation_graph_events, event_idx, num_neighbors=200)
        self.tgnn_reward_wraper.original_score = self.libcity_base_explainer.tgnne_score_func(self.candidate_events, )

    def _initialize(self, event_idx, libcity_base_explainer):
