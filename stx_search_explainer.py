...
#        print(exp_percentage_error, exp_size_percentage, fidelity_size_score)
    ...
    def check_monotonicity(self):
        self.results_dir = f"../my-libcity/results/{self.dataset_name}/stx_search/stx_search_{self.model_name}_{self.dataset_name}_{self.explaining_event_idx}_{20}.pck"

        print(self.results_dir)
        with open(self.results_dir, "rb") as f:
            results = pck.load(f)

        exp_events = results["events"]
        print(exp_events)
        ...
        reduced_events = []
        for i in range(4):
            not_in_exp = [i for i in self.candidate_events if i not in exp_events]
            # not_in_exp = copy.deepcopy(self.candidate_events)
            random.shuffle(not_in_exp)
            shuffles.append(not_in_exp)
            all_scores.append([])
            reduced_events.append(copy.deepcopy(self.candidate_events))

        for j in tqdm(range(len(shuffles[0]))):
            for i in range(4):
                reduced_events[i].remove(shuffles[i][j])
                exp_absolute_error, fidelity_size_score, delta_fidelity, model_y, target_model_y, exp_y, target_exp_y, target_complement_exp_y = self.score_func(reduced_events[i])
                all_scores[i].append(exp_absolute_error)

            if j % 250 == 0:
                fig, axes = plt.subplots(1, 1, figsize=(10, 5))
                with open(f'results/monotonicity_results/{self.dataset_name}monotonicity_check.pck', 'wb') as f:
                    pck.dump((shuffles, all_scores), f)
                for i in range(4):
                    axes.plot(range(len(all_scores[i])), all_scores[i]) 
                    axes.set_xlabel('Number of events removed from the explanation')
                    axes.set_ylabel('Absolute error of the model prediction')
                fig.savefig(f'figures/{self.dataset_name}monotonicity_check.png')
