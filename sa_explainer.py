...
class SimulatedAnnealing:
    ...
    def annealing_iteration(self, iteration=0):
        ...
        new_error, new_fidelity_size_score, new_delta_fidelity, model_y, target_model_y, exp_y, target_exp_y, target_complement_exp_y = self.objective_function(
            new_events)
        ...
        if self.expmode in ['fidelity', 'fidelity+size']:
            def delta_criteria(d): return d < 0
            def score_criteria(s, b_s): return s < b_s
...
#                self.target_complement_exp_y = target_complement_exp_y
        ...
        if self.verbose == True:
            if iteration % 100 == 0:
                ...
                print('Model pred:', self.target_model_y, 'Exp pred:',
                      target_exp_y, 'Unimportant pred:', target_complement_exp_y)
...
#            pck.dump((self.explainer, self), f)
    ...
    def add_event(self, current_events, num_events=2):
        ...
        new_event = np.random.choice(
            available_events, num_events, replace=False)
        [new_events.append(e) for e in new_event]
        return new_events

    def remove_event(self, current_events, num_events=1):
        new_events = copy.copy(current_events)
        events_to_remove = np.random.choice(
            new_events, num_events, replace=False)
...
#        new_events.append(new_event)
    ...
    def visualisation_data(self, ):
        results = {}
        print('SA TARGET IDX: ', self.target_idx)
    ...
    def run(self, iterations=10000, expmode='fidelity', best_events=None):
        ...
        if self.expmode == 'fidelity':
            initial_events = list(np.random.choice(
        ...
        self.error, self.fidelity_size_error, self.delta_fidelity, self.model_y, self.target_model_y, self.exp_y, self.target_exp_y, self.target_complement_exp_y = self.objective_function(
            initial_events)
