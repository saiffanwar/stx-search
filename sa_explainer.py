...
class SimulatedAnnealing:
    def __init__(self, data, graph_events, adj_mx, dataset_name, target_idx, candidate_events, exp_size, score_func, expmode='fidelity', verbose=False):
        self.data = data
    ...
    def annealing_iteration(self,iteration=0):
        ...
        new_error, new_fidelity_size_score, new_delta_fidelity, model_y , target_model_y, exp_y, target_exp_y, target_complement_exp_y = self.objective_function(new_events, self.target_idx)
...
#                self.target_complement_exp_y = target_complement_exp_y
                self.target_exp_y = target_exp_y
                self.target_model_y = target_model_y
        ...
        if self.verbose == True:
            if iteration%100==0:
                ...
                print('Model pred:', target_model_y, 'Exp pred:', target_exp_y, 'Unimportant pred:', target_complement_exp_y)
...
#        new_events.append(new_event)
    ...
    def run(self, iterations=10000, expmode='fidelity', best_events=None):
        ...
        if self.expmode == 'fidelity':
            ...
            with open(f'results/{self.dataset_name}/best_result_{self.target_idx}_{self.exp_size}_fidelity.pck', 'rb') as f:
        ...
        self.error, self.fidelity_size_error, self.delta_fidelity, self.model_y, self.target_model_y, self.exp_y, self.target_exp_y, self.target_complement_exp_y = self.objective_function(initial_events, self.target_idx)
...
#        print('Best score: ', self.best_score, 'Best pred:', self.best_pred, 'Best delta fidelity:', self.best_delta_fidelity, 'Best unimportant pred:', self.best_unimportant_pred)
        ...
        if len(self.best_events) != int(len(self.candidate_events)):
            for i in tqdm(range(iterations)):
                ...
                if i%100 == 0:
                    with open(f'results/{self.dataset_name}/best_result_{self.target_idx}_{self.exp_size}_{self.expmode}.pck', 'wb') as f:
