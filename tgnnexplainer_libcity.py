...
#        self.device = self.config.get('device', torch.device('cpu'))
    ...
    def construct_input_data_from_event(self, all_events, target_idx):
        ...
        self.target_event = self.Event(self.all_events.iloc[target_idx]['ts'],
                                       self.node_id_to_idx.index(
            self.all_events.iloc[target_idx]['u']),
...
#        self.model_y = self.scaler.inverse_transform(self.model_y)
    ...
    def set_computation_graph(self, edge_feat, target_idx, threshold_num=None):
        ...
        src_to_dst = edge_feat.groupby(
            'src')['destination'].apply(list).to_dict()
...
# Filter input_window_events just once
        neighbouring_node_events = self.input_window_events[self.input_window_events['u'].isin(
            neighbouring_nodes)].copy()
        self.events = []
...
#            print(e.node_idx, e.timestamp, e.value)
    ...
    def reduce_candidate_events(self, threshold_num):
        ...
        most_recent = sorted(self.candidate_events, key=lambda e: self.events[e].timestamp, reverse=True)[
            :threshold_num]
    ...
    def generate_masked_input(self, exp_events):
        ...
        for event_idx in self.computation_graph:
            if event_idx not in exp_events:
                ...
                event_timestamp, event_node_idx = self.events[
                    event_idx].timestamp, self.events[event_idx].node_idx
                masked_input['X'][0, event_timestamp,
                                  event_node_idx, :] = self.events[event_idx].value
#        return self.data
        return masked_input
...
#        exp_masked_input.to_tensor(self.device)
    ...
    def tgnne_score_func(self, exp_events):
        ...
        with torch.no_grad():
            ...
            target_model_y = self.model_y[0, 0,
                                          self.target_event.node_idx, 0].item()
...
#        masked_prediction = self.scaler.inverse_transform(masked_prediction.cpu())
        ...
        target_masked_exp_y = masked_prediction[0,
                                                0, self.target_event.node_idx, 0]
