...
def load_results(dataset="GRID", model="TGCN"):
    ...
    with open(file_path, "rb") as f:
        results = pck.load(f)
    plt.plot(list(range(len(results))), [float(r) for r in results], label="PGE Losses")
    plt.show()
...
def base_model_losses(dataset="PEMS_BAY", model="TGCN"):
    ...
    plt.plot(
        list(range(len(training_losses))),
        [float(r) for r in training_losses],
        label="Training Losses",
    )
    plt.plot(
        list(range(len(validation_losses))),
        [float(r) for r in validation_losses],
